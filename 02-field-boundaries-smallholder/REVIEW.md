...
| # | State | What changed |
|---|---|---|
| F-01 | Scoped | `COMPARISON.md` now states the floor as real within India with its cause unestablished, and carries the cross-country table as evidence. `RESULTS.md` carries the same correction against its saturation paragraph. The label registration test that would settle the cause is still not run |
| F-02 | Closed | SAM is in the write-up. O-07 and the foundation-model paragraph are gone |
| F-03 | Closed | Every table is generated by `build_comparison.py`. `check_tables.py` fails the build if a number in the document appears in no generated table |
| F-04 | Closed | Measured at 200 draws. FTW's Indian gap is +0.0063 against a null of standard deviation 0.0030, and 3 draws in 200 of random cells beat the checkpoint. Draws kept in `results/india/null_draws_ftw.csv` |
| F-05 | Closed | Chip-level bootstrap in `bootstrap_ci.py`. FTW's Slovenian interval widens 3.69 times, its Indian interval 1.45 times |
| F-06 | Closed | `recall_at_ftw_budget` is computed for all four methods by one function, with an interpolated-or-clamped flag |
